...
create t v = do
  ...
  case res of
    ...
    UnknownError                                  -> afterRollback t toK'   v  >>= return . (\x -> SaveFailed  (toArg' x) [] False)
  where
...
--save' :: (ModelClass m) => DB.Table m -> arg -> (m -> IO (Entity m)) -> (m -> arg) -> (arg -> ) -> IO (SaveResult m arg pos)
--save' t arg actionImpl' toArg'
{-
    actionImpl' x = do
      DB.repsert t k x
      DB.get t k
